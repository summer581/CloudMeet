...
static const QString g_darkStyle = R"(
    QWidget {
...
static const QString g_lightStyle = R"(
    QWidget {
    ...
    , ui(new Ui::Widget)
{
    ...
    this->setStyleSheet(g_darkStyle);
    ...
    //摄像头出错处理
    connect(_camera, SIGNAL(errorOccurred(QCamera::Error,QString)), this, SLOT(cameraError(QCamera::Error)));
    _myvideosurface = new MyVideoSurface(this);


    connect(_myvideosurface, SIGNAL(frameAvailable(QImage)), this, SLOT(cameraImageCapture(QImage)));
    ...
    _captureSession->setVideoSink(_myvideosurface->videoSink());

    // 尝试选择彩色格式，避免默认选中 Y8 灰度（必须在 setCamera 之后）
    {
        QList<QCameraDevice> allDevices = QMediaDevices::videoInputs();
        QCameraFormat bestFormat;
        for (const QCameraDevice &dev : allDevices) {
            for (const QCameraFormat &fmt : dev.videoFormats()) {
                QVideoFrameFormat::PixelFormat pf = fmt.pixelFormat();
                if (bestFormat.resolution().isEmpty() &&
                    (pf == QVideoFrameFormat::Format_ARGB8888 ||
                     pf == QVideoFrameFormat::Format_XRGB8888 ||
                     pf == QVideoFrameFormat::Format_BGRA8888 ||
                     pf == QVideoFrameFormat::Format_BGRX8888 ||
                     pf == QVideoFrameFormat::Format_RGBX8888 ||
                     pf == QVideoFrameFormat::Format_RGBA8888 ||
                     pf == QVideoFrameFormat::Format_NV12 ||
                     pf == QVideoFrameFormat::Format_YUV420P ||
                     pf == QVideoFrameFormat::Format_YUYV ||
        ...
        if (!bestFormat.resolution().isEmpty()) {
            _camera->setCameraFormat(bestFormat);
        }
    }

    // 给主显示标签设置抗闪烁属性
    ui->mainshow_label->setAttribute(Qt::WA_OpaquePaintEvent);
    ui->mainshow_label->setAttribute(Qt::WA_NoSystemBackground);
    ...
    _mentionSound = new QSoundEffect(this);
    _mentionSound->setSource(QUrl("qrc:/myEffect/2.wav"));

    ui->themeComboBox->setCurrentIndex(0);
    connect(ui->themeComboBox, QOverload<int>::of(&QComboBox::currentIndexChanged),
            this, &Widget::on_themeComboBox_currentIndexChanged);
}


void Widget::cameraImageCapture(QImage img)
{
    ...
    QSize labelSize = ui->mainshow_label->size();
    if (labelSize.width() <= 0 || labelSize.height() <= 0) return;

    QImage rotated = img.scaled(labelSize, Qt::KeepAspectRatio, Qt::FastTransformation)
                        .flipped(Qt::Horizontal);

    if(partner.size() > 1)
    {
        emit pushImg(rotated);
    }

    if(_mytcpSocket->getlocalip() == mainip)
    {
        ui->mainshow_label->setPixmap(QPixmap::fromImage(rotated));
    }

    Partner *p = partner[_mytcpSocket->getlocalip()];
    if(p) p->setpic(rotated);
}
...
void Widget::datasolve(MESG *msg)
{
    ...
    {
        quint32 roomno = 0;
        if (msg->data && msg->len >= 4)
        {
            memcpy(&roomno, msg->data, 4);
        }
        else if (msg->data && msg->len > 0)
        {
            memcpy(&roomno, msg->data, msg->len);
        }
        qDebug() << "CREATE_MEETING_RESPONSE raw bytes:" << (msg->data ? msg->data[0] : 0) << (msg->data ? msg->data[1] : 0) << (msg->data ? msg->data[2] : 0) << (msg->data ? msg->data[3] : 0) << "len=" << msg->len << "roomno=" << roomno;

        if(roomno != 0)
        {
            QMessageBox msgBox(this);
            msgBox.setWindowTitle("创建成功");
            msgBox.setText(QString("房间号：%1").arg(roomno));
            msgBox.setInformativeText("点击\"复制房间号\"可将房间号复制到剪贴板");
            QPushButton *copyButton = msgBox.addButton("复制房间号", QMessageBox::ActionRole);
            msgBox.addButton("确定", QMessageBox::AcceptRole);
            msgBox.exec();
            if (msgBox.clickedButton() == copyButton)
            {
                QClipboard *clipboard = QApplication::clipboard();
                clipboard->setText(QString::number(roomno));
            }
            ...
            WRITE_LOG("succeed creating room %u", roomno);
            //添加用户自己
        ...
        {
            _createmeet = false;
            QMessageBox::warning(this, "Room Information", QString("房间创建失败，请退出后重新连接再试"), QMessageBox::Yes, QMessageBox::Yes);
            ui->outlog->setText(QString("房间创建失败"));
            ui->createmeetBtn->setDisabled(false);
            WRITE_LOG("room creation failed, roomno=0");
        }
    }
    else if(msg->msg_type == JOIN_MEETING_RESPONSE)
    {
        qint32 c = 0;
        if(msg->data && msg->len >= sizeof(qint32))
        {
            memcpy(&c, msg->data, sizeof(qint32));
        }
        if(c == 0)
        {
    ...
    {
        if (msg->data == NULL || msg->len == 0 || msg->len % sizeof(uint32_t) != 0)
        {
            qDebug() << "PARTNER_JOIN2 invalid data, len=" << msg->len;
        }
        else
        {
            uint32_t ip;
...
Partner* Widget::addPartner(quint32 ip)
{
    ...
    else
    {
        ...
        if (ui->themeComboBox->currentIndex() == 1) {
            p->setStyleSheet("border-width: 1px; border-style: solid; border-color: #d1d5db; border-radius: 4px;");
        }
...
void Widget::removePartner(quint32 ip)
{
    if(partner.contains(ip))
    {
        ...
        if (partner.size() <= 1)
        {
            disconnect(this, SIGNAL(volumnChange(int)), _ainput, SLOT(setVolumn(int)));
            disconnect(this, SIGNAL(volumnChange(int)), _aoutput, SLOT(setVolumn(int)));
            _ainput->stopCollect();
            _aoutput->stopPlay();
            ui->openAudio->setText(QString(OPENAUDIO).toUtf8());
            ui->openAudio->setDisabled(true);
...
void Widget::on_joinmeetBtn_clicked()
{
    ...
    QRegularExpression roomreg("^[1-9][0-9]{7}$");
...
{
    bool dark = (index == 0);
    ChatMessage::setDarkTheme(dark);

    this->setStyleSheet(dark ? g_darkStyle : g_lightStyle);

    QString partnerBorder = dark
        ? "border-width: 1px; border-style: solid; border-color: #334155; border-radius: 4px;"
        : "border-width: 1px; border-style: solid; border-color: #d1d5db; border-radius: 4px;";
